...
contract ArbitrableTokenList is PermissionInterface, Arbitrable {
     ...
     */
    // Artbilea.methods.requestStatusChange(hash, 0, addressArtwork, 0)

    // const id = keccak256( //podemos acceder aca desde el front
    //         abi.encodePacked(
    //             hash, //hash
    //             0,
    //             addr, //address de ctto Artwork
    //             0
    //         )
    ...
    function requestStatusChange(
        string _name, //hash
        string _ticker, //0
        address _addr, //address de nuestro ctto Artwork
        string _symbolMultihash //0
    )
        external
        payable

    {
        bytes32 tokenID = keccak256( //podemos acceder aca desde el front
            abi.encodePacked(
                _name, //hash
                _ticker,
                _addr, //address de ctto Artwork
                _symbolMultihash
            )
        );
    ...
    function withdrawFeesAndRewards(address _beneficiary, bytes32 _tokenID, uint _request, uint _round) public {
        ...
        _beneficiary.transfer(reward); // It is the user responsibility to accept ETH.
    }
    ...
    function contribute(Round storage _round, Party _side, address _contributor, uint _amount, uint _totalRequired) internal {
        ...
        _contributor.transfer(remainingETH); // Deliberate use of send in order to not block the contract in case of reverting fallback.
    }
